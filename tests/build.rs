...
fn build_lib_invokes_opc() {
    ...
    assert!(output.exists(), "expected lib output at {}", output.display());
}
...
fn build_rom_invokes_opc() {
    ...
    let output = project
        ...
        .join(triplet)
        .join("mygame.bin");
    assert!(output.exists(), "expected rom output at {}", output.display());
}
...
"#
    ...
    assert!(carts.join("Cart.toml").exists(),
        "expected dep-lib cloned into carts dir at {}", carts.display());
    assert!(project.join("Cart.lock").exists(), "expected Cart.lock written");
}
